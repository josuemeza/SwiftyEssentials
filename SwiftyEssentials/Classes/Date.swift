...
extension Date {
     ...
     */
    static func parse(stringDate: String, format: String, fromTimeZone: String = "GMT", toTimeZone: String? = nil) -> Date? {
    ...
    static func initFromString(format: String, date: String) -> Date? {
        return Date.parse(stringDate: date, format: format)
    }
     ...
     */
    func string(format: String, locale: String = "es_CL", withTimezone timezone: String = TimeZone.current.identifier) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = format
